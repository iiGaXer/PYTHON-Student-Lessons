...
with open(file_name) as file:
    ...
    for row in reader:
        Predictions = int(row[0])
        Y_true = int(row[1])
        sub_data = int(row[2])
        sub_data_2 = int(row[3])

        Data_2.append(sub_data_2)
        Data_1.append(sub_data)
...
Avg = [np.mean(Data_1), np.mean(Data_2)]

for i in range(len(Avg)):
  if i == 0:
    for a in range(len(Data_1)):
      Data_1[a] = Data_1[a] - Avg[i]

  elif i == 1:
    for b in range(len(Data_1)):
      Data_2[b] = Data_2[b] - Avg[i]
...
chart.render_to_file('COVID Training.svg')